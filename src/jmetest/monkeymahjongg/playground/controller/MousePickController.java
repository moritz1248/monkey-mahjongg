...
import jmetest.monkeymahjongg.playground.view.UITile;
...
public class MousePickController extends MahjonggGameController {
    ...
    public void update(float time) {
        ...
        if (released && pick.getValue() > 0) {
            ...
            if (pr.getNumber() > 0) {
				Geometry geometry = pr.getPickData(0).getTargetMesh();
                Tile tile = (Tile) geometry.getUserData(UITile.TILE_USER_DATA);
                tile.select();
			}
        }
    }
